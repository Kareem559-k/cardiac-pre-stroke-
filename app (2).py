...
def make_probability_bar(prob, severity):
    ...
    colors = {"high":"#ffffff","medium":"#ffffff","low":"#ffffff"}
    ax.barh(["Risk"], [prob], color=colors[severity], height=0.5)
    ax.set_xlim(0,1)
    ax.set_yticks([])
    ax.set_xticks([0,0.25,0.5,0.75,1])
    ax.set_xlabel("Risk Level", color="white")
    ax.text(prob, 0, f"{prob*100:.1f}%", va='center', fontsize=12, fontweight='bold', color='white')
...
if hea_file and dat_file:
    ...
    try:
        ...
        if len(y) > 50:
            ...
            ax3.plot(rms[-200:], color="white", linewidth=1.0)
            ax3.set_yticks([])
            ax3.set_xticks([])
            fig3.patch.set_facecolor("#020c1b")
            st.pyplot(fig3)
            plt.close(fig3)
    ...
    color_bg = {"high":"#1c1c3c","medium":"#1c1c3c","low":"#1c1c3c"}[severity]
    st.markdown(f"""
    <div style='background:{color_bg};padding:16px;border-radius:12px;text-align:center;font-size:16px;color:white'>
