...
if hea_file and dat_file:
    ...
    tabs = st.tabs([
        ...
        "Spectrogram", "Risk", "ROC Curve", "Diagnosis"
    ])
    ...
    with tabs[5]:
        ...
        ax.set_xlim(0, 1)
        ax.set_yticks([])
        ax.set_xlabel("Probability", color="gray")
    ...
    # ----------------- DIAGNOSIS -----------------
    with tabs[7]:
        st.subheader("🧠 Diagnosis Result" if lang=="English" else "🧠 نتيجة التشخيص")
        if prob < 0.3:
            color = "#2E8B57"
            label = "Normal" if lang=="English" else "القلب سليم"
        elif prob < 0.6:
            color = "#FFD700"
            label = "Borderline" if lang=="English" else "حالة متوسطة"
        else:
            color = "#FF6347"
            label = "High Risk" if lang=="English" else "خطر مرتفع"
...
# ----------------- FOOTER -----------------
st.markdown("""<hr style="border:1px solid #333;">
